...
const VehicleScene=forwardRef<SceneHandle,Props>(function VehicleScene(props,ref){
 ...
 useEffect(()=>{
  ...
  // Độ nét: xe nguyên khối (explode = 0) render tới 2× DPR để zoom thật nét; hễ bắt đầu tách rời thì hạ về
  // 1,5× (desktop) / 1,25× (cảm ứng) vì hàng trăm chi tiết + nhãn nặng hơn nhiều. Quy tắc cố định, không tự hạ theo FPS.
  const coarse=window.matchMedia('(pointer: coarse)').matches;const dpr=window.devicePixelRatio||1;
  let pixelRatio=0;
  const applyPixelRatio=(explode:number)=>{const want=Math.min(dpr,explode<=0?2:(coarse?1.25:1.5));if(Math.abs(want-pixelRatio)>.01){pixelRatio=want;renderer.setPixelRatio(pixelRatio);renderer.setSize(el.clientWidth||1,el.clientHeight||1)}};
